...
fn test_arithmetic_complete() {
    ...
    // Undo SUB and ADD
    ctx.undo().check_reg("x4", "0")
       .undo().check_reg("x3", "0");
    
    // Redo both
    ctx.redo().check_reg("x3", "80")
       .redo().check_reg("x4", "20");
}
...
fn test_logical_operations() {
    ...
    ctx.exec("AND x3, x1, x2")         // 0x0F
       ...
       .check_reg("x3", "15")          // 0x0F
       .check_reg("x4", "255")         // 0xFF
       .check_reg("x5", "240");        // 0xF0
...
fn test_memory_all_sizes() {
    ...
    ctx.exec("ADDI x2, x0, 0xFF")
       ...
       .check_reg("x3", "4294967295")  // Sign-extended 0xFF
       .check_reg("x4", "255");        // Zero-extended
...
fn test_upper_immediate() {
    ...
    // Test LUI
    ctx.exec("LUI x1, 0x12345")
       .check_reg("x1", "305418240");  // 0x12345000
    
    // Test AUIPC (PC-relative)
    ctx.exec("AUIPC x2, 0x1000");      // PC + (0x1000 << 12)
    
    // Undo both
    ctx.undo().check_reg("x2", "0")
       .undo().check_reg("x1", "0");
}
...
fn test_memory_aliasing() {
    ...
    ctx.exec("LW x5, 0(x1)")
       .check_reg("x5", "305419896");  // 0x12345678
}
...
fn test_maximum_undo_redo_cycles() {
    ...
    // Verify final state
    for i in 1..=5 {
        ctx.check_reg(&format!("x{}", i), &format!("{}", i * 10));
    }
