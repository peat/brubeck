...
fn test_undo_csr_operations() {
    ...
    ctx.exec("ADDI x1, x0, 1234")
       .exec("CSRRW x2, 0x340, x1");  // Write to mscratch
...
fn test_pseudo_instruction_undo() {
    ...
    ctx.exec("LI x1, 0x12345")
       .exec("MV x2, x1");
    
    // Verify state
    ctx.check_reg("x2", "74565");     // 0x12345
